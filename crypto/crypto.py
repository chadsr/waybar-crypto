...
abs_dir = os.path.dirname(
    os.path.abspath(__file__))  # Get the absolute path of this script
...
try:
    ...
    print(
        f'Could not find/open crypto.ini ({config_path}). Does it exist?\n{e}')
    sys.exit(1)

# Attempt to parse required fields of crypto.ini
try:
    coins = [section for section in config.sections() if section != 'general'
